...
while (my $linea = <MIA>){
	...
	if ($linea !~ /#/ && $linea =~ /\w/){
		my @a = split (/\t/, $linea);
		$a[1] =~ tr/A-Z/a-z/;
		if ($a[0] ne 'recency' && $a[0] ne 'clade_membership' && $a[0] ne 'emerging_lineage' && $a[0] ne 'pango_lineage'){
			$hashCo{$a[1]} = $a[0];
			#print ("->($a[1])\n");
		}
	}
...
while (my $linea = <MIA>){
	...
	if ($l > 1){
		...
		#my @c = split (/\//, $a[6]);
		my @b = ();
		push (@b, $a[5]) if ($a[5] =~ /\w/); # region
		push (@b, $a[6]) if ($a[6] =~ /\w/); # country
		push (@b, $a[7]) if ($a[7] =~ /\w/); # division
		push (@b, $a[8]) if ($a[8] =~ /\w/); # location
		my @c = @b;
		for (my $i = 0; $i <= $#b; $i++){
			$b[$i] =~ s/^\s+//;
			$b[$i] =~ s/\s+$//;
			...
			for (my $i = 0; $i <= $#b; $i++){
				if (!exists $hashCo{$b[$i]}){
					if (!exists $check{$gc}){
						...
						print ("context in $fileMe: $gc\n");
						$output{$gc} = $c[$i];
						$check{$gc} = 1; # With this hash I control whether the name (and its geographic context) is already identified
					}
				}
			}
